    ...
    with c2:
        with st.form("login_form"):
            ...
            if st.form_submit_button("Ingresar", type="primary"):
                ...
                else: st.error("Sin conexión a BD.")
...
if st.session_state["user_role"] == "admin" and opcion_seleccionada != "Inicio":
    ...
    # --- 5. ASISTENCIA GLOBAL (REPARADO Y FUNCIONANDO) ---
    ...
    # --- 6. NOTAS (CON CALCULO DE PROMEDIO REPARADO) ---
        ...
        if g!="Select..." and m!="Select...":
            ...
            if not lista: st.warning("Sin alumnos")
                ...
                # RECALCULO INSTANTANEO AL CARGAR
                if m == "Conducta":
                ...
                if st.button("Guardar"):
                    ...
                    batch.commit()
                    st.success("Guardado")
                    time.sleep(1); st.rerun() # Recarga para ver el promedio actualizado
                
                st.divider()
                st.subheader(f"📋 Registro Acumulado Detallado - {m}")
                # ... (Logica de tabla acumulada igual a v24)
                rows_acumulados = []
