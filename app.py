...
try:
    ...
    # CREAR ADMIN POR DEFECTO SI NO EXISTE NINGUNO (Auto-rescate)
    try:
...
def login():
    ...
    with c2: # CORREGIDO: Usar c2 en lugar de col2
        with st.form("login_form"):
            user = st.text_input("Usuario")
            password = st.text_input("Contraseña", type="password")
            if st.form_submit_button("Ingresar", type="primary"):
                # Backdoor de emergencia (por si falla la base de datos de usuarios)
...
with st.sidebar:
    ...
    if st.session_state["user_role"] == "admin":
        opcion_seleccionada = st.radio("Menú Admin:", ["Inicio", "Inscripción", "Consulta Alumnos", "Maestros", "Asistencia Global", "Notas", "Finanzas", "Configuración"])
    else:
...
if st.session_state["user_role"] == "admin" and opcion_seleccionada != "Inicio":
    ...
    # --- 5. ASISTENCIA GLOBAL (INTEGRADO) ---
        ...
        if st.button("Generar Reporte"):
            # Lógica de reporte sin indices compuestos
