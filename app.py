...
try:
    ...
    # Auto-creación de Admin de Rescate si la BD está vacía (Solo primera vez)
    try:
...
# ==========================================
# 4. BARRA LATERAL (CON ROLES REALES)
# ==========================================
with st.sidebar:
    ...
    # MENÚ SEGÚN ROL - INTEGRADO
    if st.session_state["user_role"] == "admin":
        opcion_seleccionada = st.radio("Menú Admin:", 
            ["Inicio", "Inscripción", "Consulta Alumnos", "Maestros", "Asistencia (Global)", "Notas", "Finanzas", "Configuración (Usuarios)"])
    else:
        # MENÚ DOCENTE
    ...
    if st.session_state.last_page != opcion_seleccionada:
        keys_to_clear = ["alum_view", "recibo", "pa", "recibo_temp", "pago_alum", "gasto_temp"]
...
if st.session_state["user_role"] == "admin" and opcion_seleccionada != "Inicio":
    ...
    # --- CONSULTA ALUMNOS ---
        ...
        if "alum_view" in st.session_state:
            ...
            # (CONTENIDO EXACTO DE TU VERSIÓN ANTERIOR)
            with tabs[0]:
    ...
    # --- 5. ASISTENCIA (NUEVO MODULO) ---
        ...
        if st.button("Generar Reporte"):
            # Lógica de reporte en Python para evitar FailedPrecondition
    ...
    if opcion_seleccionada == "Mis Listados":
        ...
        if g!="Select..." and m!="Select...":
            ...
            if not lista: st.warning("Sin alumnos")
                ...
                df["Promedio"] = 0.0
                cfg = {"NIE": st.column_config.TextColumn(disabled=True), "Nombre": st.column_config.TextColumn(disabled=True, width="medium"), "Promedio": st.column_config.NumberColumn(disabled=True)}
                for c in cols: cfg[c] = st.column_config.NumberColumn(min_value=0, max_value=10.0, step=0.01)
                ed = st.data_editor(df, column_config=cfg, hide_index=True, use_container_width=True, key=id_doc)
                if st.button("Guardar"):
                    batch = db.batch()
                    detalles = {}
