...
with st.sidebar:
    ...
    if st.session_state["user_role"] == "admin":
        opcion = st.radio("Menú Admin:", ["Inicio", "Inscripción", "Maestros", "Consulta Alumnos", "Finanzas", "Notas (Admin)", "Configuración"])
    else:
    ...
    st.markdown("---")
    st.caption("© 2026 David Fuentes | Licencia: Colegio CBEH")
...
if st.session_state["user_role"] == "admin" and opcion != "Inicio":
    ...
    # --- CONSULTA (CON BOLETA) ---
        ...
        if "alum_view" in st.session_state:
            ...
            t1, t2 = st.tabs(["Historial", "🖨️ Boleta de Notas"])
            with t1: st.info("Historial de pagos y documentos disponible en módulos específicos.")
